...
/* function that creates a hash table (0)*/
hash_table_t *hash_table_create(unsigned long int size);

/* function to implement the djb2 algorithm (1)*/
unsigned long int hash_djb2(const unsigned char *str);

/* function to return the index of a given key (2)*/
unsigned long int key_index(const unsigned char *key, unsigned long int size);

/* function that adds an element to the hash table (3)*/
int hash_table_set(hash_table_t *ht, const char *key, const char *value);
/* declaration of subfunction to duplicate the key/value string */
char *duplicate_string(const char *string);
/* helper function to create and initialize new node of hash table*/
hash_node_t *initialize_new_node(char *key_duplicate, char *value_duplicate);
/* helper function to free dups if exit failure or node re-set */
void free_dups(hash_node_t **new_node, char **key, char **value);

/* function that returns the value associated with a key (4)*/
char *hash_table_get(const hash_table_t *ht, const char *key);

/* function to print all key/value pairs of a hash table (5)*/
void hash_table_print(const hash_table_t *ht);

/* function to delete a hash table (6)*/
